...
was: GetResult getResult = collection.get("key", getOptions().timeout(Duration.ofSeconds(3));
----
...
Builds can be found on Nuget. 
In addition, a `zip` file is available with the artifacts. 
Please see the xref:sdk-release-notes.adoc[Release Notes] for up-to-date information.

IMPORTANT: .NET SDK 3.x has a minimum required .NET Standard version of 2.1, although we recommend running the latest version.
...
Otherwise please download the zip file and update the artifacts accordingly.


=== Configuring the Environment

Environment configuration from SDK 2 remains the same, although somewhat more flexible, with its configuration cleaned up to be more easily discoverable. 
//TODO: The `CouchbaseEnvironment` has been renamed to `ClusterEnvironment` and can be customized through a builder:
...
If you are using a Couchbase Server version which does not support `Collections`, always use the `defaultCollection()` method to access the KV API; it will map to the full bucket transparently in the background.
...
Also note you will now find Query, Search, and Analytics at the `Cluster` level -- this is where they logically belong. 
...
ITODO: a note on API names changing
